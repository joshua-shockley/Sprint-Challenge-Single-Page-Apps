...
> Answer: using component renduring so that the entire page doesnt need to render from server side making for a faster response time between site pages..
- [ ] What does AJAX stand for?
> Answer: its the acronym for asynchronous javascript. its a technique
- [ ] What are `controlled components` in React?
> Answer: controlled component is one that sets state as the "source of truth"
